...
- 팔레트를 고양이에게 드롭하면 팔레트 창 닫기
- 고정(핀), 검색/필터, 삭제, 전체 지우기, 드래그로 순서 재배치
- 전역 단축키 `Ctrl+Shift+V` 또는 펫 래디얼 메뉴 "클립보드"로 열기
- 팔레트는 항상 위 + 타이틀바 핸들로 다른 모니터로 이동 가능

> **macOS 화면 캡처는 클립보드에 올라와야 팔레트에 잡힙니다.**
> macOS 기본 단축키(`⌘⇧4`/`⌘⇧3`)는 **파일로만 저장**되고 클립보드엔 안 들어가서, 캡처해도 팔레트에 안 뜹니다.
>
> - **바로 쓰기**: `Ctrl`을 더한 `⌘⌃⇧4`(영역) / `⌘⌃⇧3`(전체) → 클립보드로 복사돼 자동으로 팔레트에 나타남. (Windows `Win+Shift+S`와 동일)
> - **습관 유지**(`⌘⇧4` 그대로 쓰고 싶으면): 시스템 설정 → 키보드 → 키보드 단축키 → 스크린샷에서
>   **"선택 영역 그림을 클립보드로 복사"** 를 `⌘⇧4`로 재지정하세요. 평소 키가 클립보드로 갑니다.
